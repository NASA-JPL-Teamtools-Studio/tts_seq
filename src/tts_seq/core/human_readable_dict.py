...
class HumanReadableDict(SeqDict):
    """
    Reader and container for standard .seq (dot-seq) sequence files common
    on Earth orbiting missions.
    
    This class handles the parsing of legacy ASCII sequence files, which use 
    positional arguments and special character-based metadata (semicolons). 
    It leverages a Command Dictionary to map positional values found in the 
    ...
    def __init__(self, file_path: pathlib.Path, config: dict) -> SeqDict:
        """
        Initializes the DotSeqDict by parsing the .seq file into a structured format.

        :param file_path: The path to the .seq file to be read.
        ...
        :rtype: SeqDict
        :raises FileNotFoundError: If the command dictionary or sequence file is missing.
        :raises ValueError: If the sequence is missing a required ID or contains conflicting keys.
        """
        ...
        dict_data = self.dotseq_to_seqjson_style_dict(file_path, cmd_dict_path)
    ...
    def build_command_stem_to_argdata_map(self, cmd_dict_path: pathlib.Path) -> dict:
        """
        Parses the XML Command Dictionary to create a lookup map for argument metadata.

        :param cmd_dict_path: Path to the mission XML command dictionary.
        :type cmd_dict_path: pathlib.Path
        :return: A dictionary mapping command stems to lists of argument definitions.
        :rtype: dict
        """
        ...
        for command in etree.parse(cmd_dict_path).findall('command_definitions/fsw_command'):
            ...
            for argument in command.findall('arguments/*'):
                arg_dict = dict()
                if argument.tag in ['integer_arg', 'float_arg', 'unsigned_arg', 'repeat_arg']:
    ...
    def extract_unquoted_comment_with_leading_ws(self, line):
        ...
        :param line: The raw line string from the .seq file.
        :type line: str
    ...
    def dotseq_to_seqjson_style_dict(self, file_path: pathlib.Path, cmd_dict_path: pathlib.Path) -> dict:
        """
        The primary parsing engine that converts a .seq file into a JSON-compatible dictionary.

        This method identifies metadata lines, time tags (Absolute/Relative), and command 
        stems. It uses the command dictionary to map positional arguments in the text file 
        to named arguments with correct types.

        :param file_path: Path to the input .seq file.
        :type file_path: pathlib.Path
        :param cmd_dict_path: Path to the command dictionary for argument mapping.
        :type cmd_dict_path: pathlib.Path
        :return: A dictionary structure compatible with standard sequence JSON ingestion.
        :rtype: dict
        """
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as dotseq_file:
                command_stem_to_argdata_map = self.build_command_stem_to_argdata_map(cmd_dict_path)
                for line in dotseq_file.read().split('\n'):
                    description = self.extract_unquoted_comment_with_leading_ws(line).replace('\n','')
                    line = line.strip()
                    if line[:2] in [';#', '']:
                        ...
                        try:
                            if ';' in line:
                                ...
                                argvals = next(csv.reader(StringIO(matches[0][0]), delimiter=self.ARG_DELIMITER))
                            else:
                                argvals = next(csv.reader(StringIO(linesplit[2]), delimiter=self.ARG_DELIMITER))
                            argvals = [x.strip().replace('"', '').replace('\'', '') for x in argvals]
                            argvals = [x for x in argvals if x != '']
                        except:
                            argvals = []
                        ...
                        try:
                            ...
                            print(f'{stem} does not appear in command dicitonary')
                            pdb.set_trace()
                            sys.exit(1)
                        if len(argmetadata) != len(argvals):
                            print(f'Mismatch between arguments supplied for {stem} in sequence file and in dictionary')
                            pdb.set_trace()
                            sys.exit(1)
